...
// Asegúrate que la ruta al JSON sea correcta, relativa a este archivo
...
export default function HomePage() {
  ...
  const hasMoreRef = useRef(true); // Para controlar si hay más por cargar

  // Procesa los datos crudos del JSON una sola vez
  const allProperties = useMemo(() => {
    return (propertiesData as any[]).map((prop): PropertyData => {
      // Intenta parsear customFields si es string, si no, usa el objeto
       let customFields = {};
       try {
  ...
  useEffect(() => {
   ...
   // Actualiza si hay más propiedades cuando cambia el displayCount o filteredProperties
   useEffect(() => {
     hasMoreRef.current = displayCount < filteredProperties.length;
   }, [displayCount, filteredProperties.length]);

   // Resetea el displayCount cuando cambian los filtros
  useEffect(() => {
    setDisplayCount(12); // Vuelve a mostrar solo las primeras 12 al filtrar
  }, [filteredProperties]);

  return (
    // El layout está en layout.tsx
    <div className="container mx-auto px-4 mt-8">
      <div className="mb-8 sticky top-[88px] z-10 bg-gray-50 py-4"> {/* Ajusta 88px a la altura real de tu header */}
          <SearchForm 
